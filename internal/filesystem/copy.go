...
const (
	...
	COPYFILE_PRESERVE_DST_TRACKED = (1<<28)
...
func Copy(source string, destination string, flags uint32) error {
	...
	result, err := C.copyfile(sourceCstr, destinationCstr, nil, C.uint(flags))
	if result != 0 {
		return err
	}
